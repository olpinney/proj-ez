...
SEARCH_CLEANED={None:None,'Assault':'Assault / Fight','Break_In':'Break In','Gun':'Gun Related',\
    'Harassment':'Harassment','Police':'Police Related','Pursuit':'Pursuit / Search','Theft':'Robbery / Theft','Weapon':'Weapon'}
...
def citizen_backfill(search=None,table_name=TABLE_NAME,file_path="olpinney/data"):
    ...
    for file in file_names:
        if re.match(f"^{table_name_w_search}_\d.*.csv$",file):
            ...
            df_csv["_geoloc"]=df_csv["_geoloc"].apply(lambda x: [json.loads(x[1:-1].replace("'", "\""))])
            #make categories into string from string represetnation of a single entry list
            df_csv["categories"]=df_csv["categories"].apply(lambda x: x[1:-1].replace("'",""))
            df_clean=clean_citizen(df_csv)
            sql_df=pd.concat([sql_df,df_clean])

    overwrite_sql_w_dedup(sql_df,search,table_name)

def citizen_combine_searches(table_name=TABLE_NAME):
    '''
    combine sql files for all citizen tables

    Input:
        table_name (str): name of table in csv and sql
        
    '''
    to_combine=SEARCH_CLEANED
...
def citizen_dedupe_sql(new_df,search=None,table_name=TABLE_NAME):
    ...
    df_combined=pd.concat([old_df,new_df])
    overwrite_sql_w_dedup(df_combined,search,table_name)
    
def overwrite_sql_w_dedup(df_new,search=None,table_name=TABLE_NAME):
    '''
    ...
    table_name_w_search = add_cleaned_search(search,table_name)
    dedupded_df=df_new.drop_duplicates(subset='objectID',keep='last')
...
def citizen_get_sql(search=None,table_name=TABLE_NAME):
    ...
    df_sql=df_sql.drop(columns='index')
...
def clean_citizen(df_citizen):
    '''
    removes extra columns from citizen data, and fixes lat and long
    '''
    df_citizen['lat']=df_citizen["_geoloc"].apply(lambda x: x[0]['lat']) 
    df_citizen['long']=df_citizen["_geoloc"].apply(lambda x: x[0]['lng']) 
    df_citizen['categories']=df_citizen['categories'].apply(lambda x: ", ".join(x) if isinstance(x,list) else x)

    to_drop=['_geoloc','updates','city_code','ranking.level',\
        'ranking.has_video','ranking.views','ranking.notifications','_highlightResult']
    df_citizen=df_citizen.drop(columns=to_drop)

    return df_citizen

def pull_recent(starting_size=100,search=None,table_name=TABLE_NAME):
    '''
    ...
    Inputs:
        search (str): optional filter term, default None
        table_name (str): name of table in sql
        starting_size (int): how many incidents to pull for first itteration
    ...
    #pull in old data
    try:
        query= f"select created_at from {table_name_w_search}" 
        #specifically for the citizen table
        old_dates=cursor.execute(query).fetchall()
    ...
    #pull in new data
    new_df=get_incidents_chicago(starting_size,search)

    while max_old_date < min(new_df['created_at']) and \
        limit*PULL_SCALING<=1000: #API restricts pulls at 1000

        print(f"max old date is {convert_dt(max_old_date)} \
            while min date is {convert_dt(min(new_df['created_at']))}")
...
def get_incidents(lat_long,limit,search=None):
    ...
    api_url="https://citizen.com/api/incident/search?insideBoundingBox[0]={}\
        &insideBoundingBox[1]={}&insideBoundingBox[2]={}&insideBoundingBox[3]={}&limit={}"
    params=list(lat_long)
    params.append(limit)
    if search:
        api_url+="&q={}&"
        try:
